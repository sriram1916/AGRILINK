...
constexpr uint32_t kControlPeriodMs = 1000;

// Safety boundaries (Phase 6 will add a real safety supervisor; for
// now the control layer self-clamps to keep the simulation bounded).
constexpr double kMaxSpeedMps       = 5.0;
constexpr double kMinSpeedMps       = -3.0;     // reverse allowed, bounded
constexpr double kMaxSteeringDeg    = 30.0;
constexpr double kRawRejectSpeedMps = 10.0;     // above this raw value -> REJECT
...
ControlCommandPayload generate_next_command(uint32_t tick_ms) {
    ...
    if (g_inject_invalid_cmd) {
        ...
        bad.desired_speed_mps = 50.0;   // >> kRawRejectSpeedMps (10.0)
...
ControlOutput validate_and_clamp_command(const ControlCommandPayload &raw) {
    ...
    if (raw.desired_speed_mps > kRawRejectSpeedMps ||
        raw.desired_speed_mps < -kRawRejectSpeedMps) {
        ESP_LOGW(TAG,
    ...
    // Clamp speed.
    if (raw.desired_speed_mps > kMaxSpeedMps) {
        out.commanded_speed_mps = kMaxSpeedMps;
        out.clamped_speed       = 1;
    } else if (raw.desired_speed_mps < kMinSpeedMps) {
        out.commanded_speed_mps = kMinSpeedMps;
        out.clamped_speed       = 1;
    } else {
    ...
    // Clamp steering.
    if (raw.steering_deg > kMaxSteeringDeg) {
        out.commanded_steering_deg = kMaxSteeringDeg;
        out.clamped_steering       = 1;
    } else if (raw.steering_deg < -kMaxSteeringDeg) {
        out.commanded_steering_deg = -kMaxSteeringDeg;
        out.clamped_steering       = 1;
    } else {
